...
# Reasons python-engineio hands to the "disconnect" handler. A phone that loses
# Wi-Fi/connectivity goes quiet without ever telling the server, so the socket
# only drops once the ping times out (or the transport itself errors). A phone
# whose browser tab is refreshed or closed sends/receives a clean shutdown, so
# the server sees "transport close"/"client disconnect" instead. We use that
# distinction to decide whether the drop looks like lost connectivity (lock
# after a grace period) or a page refresh (unpair after a shorter grace period).
...
class SecureSocketServer:
    def __init__(self,socketio,services):
        ...
        # session_id -> {"since": monotonic seconds, "action": "lock"|"unpair", "reason": str}
    ...
    def process_pending_disconnects(self,*,now=None):
        # Called from Services.watchdog() every ~0.1s so the grace periods
        # below apply regardless of which thread/greenlet handled the
        # socket.io "disconnect" event.
        if not self.pending_disconnects: return
        current=time.monotonic() if now is None else now
        for session_id,pending in list(self.pending_disconnects.items()):
            active=self.services.sessions.active()
            if active is None or active.session_id!=session_id:
                # Session already gone (e.g. unpaired some other way); nothing left to do.
                self.pending_disconnects.pop(session_id,None); continue
            elapsed=current-pending["since"]
            if pending["action"]=="lock" and elapsed>=self.services.config.connection_lost_lock_delay_seconds:
                self.pending_disconnects.pop(session_id,None)
                self.services.lock_now(reason=pending["reason"])
            elif pending["action"]=="unpair" and elapsed>=self.services.config.phone_refresh_unpair_delay_seconds:
                self.pending_disconnects.pop(session_id,None)
    ...
    def register(self):
        ...
        def disconnect(reason=None):
            ...
            if reason in _CONNECTIVITY_LOST_REASONS:
                # Phone went quiet without closing the socket: likely Wi-Fi/connection loss.
                # Give it a grace period to come back before locking the workstation.
                self.pending_disconnects[session_id]={"since":time.monotonic(),"action":"lock","reason":f"phone connection/Wi-Fi lost ({reason})"}
            else:
                ...
                self.pending_disconnects[session_id]={"since":time.monotonic(),"action":"unpair","reason":f"phone socket disconnected ({reason or 'tab closed or refreshed'})"}
        ...
        def secure_message(raw):
            try:
                ...
                self.pending_disconnects.pop(session.session_id,None)
